...
2. 「請求先マスタ」に請求先、「請求明細」に対象年月・品目・単価を入力
3. メニュー **［請求書］→［請求書PDFを作成］** で試し、問題なければ **［毎月の自動作成をONにする］**

- 自社情報・住所・振込先・備考は実際の請求書に合わせて設定済み（`gas/InvoiceConfig.gs`）。
  **インボイスの登録番号だけ空欄**なので、必要なら `ISSUER.REGISTRATION_NO` に入力してください
- 既定は **毎月25日発行・当月末払い**（`TRIGGER_DAY_OF_MONTH` / `DEFAULT_DUE_RULE` で変更可）
- 保存先は「請求書PDF / 2026年8月 / 8月分請求書:〇〇株式会社様.pdf」。
  **毎月手動で作っているフォルダがあれば、同じ名前のフォルダにそのまま保存**します（`MONTH_FOLDER_FORMAT`）
- 値引き行（単価にマイナス）、軽減税率8%の区分表示に対応
