...
function writeAdTargets_(sheet, targets) {
  ...
  [[AD_ACCOUNT_ROW, 'Meta 広告アカウントID', targets.account,
   ...
   [AD_CAMPAIGN_ROW, 'キャンペーン名に含む文字', targets.campaign,
    'このクライアントの採用広告だけを取り込むための絞り込み。スペース区切りで複数指定でき、すべて含むものだけが対象（例：あおき屋 採用）']
  ].forEach(function (t) {
...
function writeAdNotes_(sheet) {
  ...
  var notes = [
    ...
    ['1つの広告アカウントで複数のクライアントや、集客広告と採用広告をまとめて回している場合は、6行目で絞ってください。'],
    ['スペース区切りで複数書けます（例：あおき屋 採用）。書いた言葉をすべて含むキャンペーンだけが対象になります。'],
    ['空のままだと、そのアカウントの全キャンペーンが混ざり、採用単価が実態と合わなくなります。'],
    ['絞り込みが効いているかは、メニューの「Meta広告の設定を確認する」で確かめられます。'],
    [''],
    ['■ 指標の意味'],
