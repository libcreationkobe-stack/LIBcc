...
function buildReviewSheet() {
  ...
  sheet.getRange(note + 2, 1).setValue(
    'ランク列はKPIシートの合計行から自動で入ります。4・5行目の目安に対する評価です。'
    + '厳しさはKPIシート2行目の「評価：〜」で変えられます（既定は甘め）。'
...
function buildReviewPrompt_(data) {
  ...
  if (data.rank) {
    lines.push('', '【今月のランク】' + rankText_(data.rank),
      'シートの4・5行目に置いた目安に対する評価です。指標ごとに 良い2点／普通1点／悪い0点で採点し、'
      + '100点満点に直したものが運用スコアで、50点が「普通」の帯にあたります。'
      + 'この目安は業界統計ではなく運用上の目標値なので、「業界平均」「業界基準」とは書かないでください。'
